...
class UploadConnector(Resource):
    @api.expect(
        api.model(
            "ConnectorUploadModel",
            {
                ...
                "source": fields.String(required=True, description="Source type (google_drive, github, etc.)"),
    ...
    def post(self):
        ...
        try:
            ...
            if data["source"] == "github":
                ...
                if not session_token:
                    return make_response(
                        jsonify(
                            {"success": False, "error": f"Missing session_token in {data['source']} configuration"}
                        ),
                        400,
                    )
            ...
            current_app.logger.error(f"Error uploading connector source: {err}", exc_info=True)
...
class DeleteConnectorSource(Resource):
    ...
    def delete(self):
        ...
        if not source_id:
            return make_response(jsonify({"success": False, "message": "source_id is required"}), 400)
        try:
            result = sources_collection.delete_one({"_id": ObjectId(source_id), "user": decoded_token.get("sub")})
            if result.deleted_count == 0:
                return make_response(jsonify({"success": False, "message": "Source not found"}), 404)
        except Exception as err:
            current_app.logger.error(f"Error deleting connector source: {err}", exc_info=True)
            return make_response(jsonify({"success": False}), 400)
...
class ConnectorsCallback(Resource):
    ...
    def get(self):
        ...
        try:
            ...
            if error:
                return redirect(
                    f"/api/connectors/callback-status?status=error&message=OAuth+error:+{error}.+Please+try+again+and+make+sure+to+grant+all+requested+permissions,+including+offline+access.&provider={provider}"
                )

            if not authorization_code:
                return redirect(
                    f"/api/connectors/callback-status?status=error&message=Authorization+code+not+provided.+Please+complete+the+authorization+process+and+make+sure+to+grant+offline+access.&provider={provider}"
                )
            ...
            try:
                ...
                return redirect(
                    f"/api/connectors/callback-status?status=success&message=Authentication+successful&provider={provider}&session_token={session_token}&user_email={user_email}"
                )

            except Exception as e:
                current_app.logger.error(f"Error exchanging code for tokens: {str(e)}", exc_info=True)
                return redirect(
                    f"/api/connectors/callback-status?status=error&message=Failed+to+exchange+authorization+code+for+tokens:+{str(e)}&provider={provider}"
                )

        except Exception as e:
            current_app.logger.error(f"Error handling connector callback: {e}")
            return redirect(
                f"/api/connectors/callback-status?status=error&message=Failed+to+complete+connector+authentication:+{str(e)}.+Please+try+again+and+make+sure+to+grant+all+requested+permissions,+including+offline+access."
            )
...
class ConnectorRefresh(Resource):
    @api.expect(
        api.model(
            "ConnectorRefreshModel",
            {"provider": fields.String(required=True), "refresh_token": fields.String(required=True)},
        )
    )
    ...
    def post(self):
        try:
            ...
            if not provider or not refresh_token:
                return make_response(
                    jsonify({"success": False, "error": "provider and refresh_token are required"}), 400
                )
    ...
    def post(self):
        try:
            ...
            documents = loader.load_data(
                {"limit": limit, "list_only": True, "session_token": session_token, "folder_id": folder_id}
            )
...
class ConnectorValidateSession(Resource):
    ...
    def post(self):
        try:
            ...
            return make_response(
                jsonify(
                    {"success": True, "expired": is_expired, "user_email": session.get("user_email", "Connected User")}
                ),
                200,
            )
        except Exception as e:
